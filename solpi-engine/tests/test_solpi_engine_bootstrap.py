"""Tests for solpi_engine_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports solpi_engine_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported solpi_engine_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("solpi_engine_bootstrap", None)
    import solpi_engine_bootstrap  # noqa: WPS433
    return solpi_engine_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import solpi_engine_bootstrap as its
    ...
    # import solpi_engine_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import solpi_engine_bootstrap' and that
        ...
        ``solpi_engine_bootstrap`` as a top-level module) leaves hermes
        ...
        # at tests/test_solpi_engine_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when solpi_engine_bootstrap hasn't been
        ...
        assert first_import_name == "solpi_engine_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'solpi_engine_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import solpi_engine_bootstrap' line to be the first import."
        )
