...
def _make_hermes_tree(root: Path) -> None:
    ...
    (root / "solpi_engine_state.db").write_bytes(b"fake-state")
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only solpi_engine_state.db (old wrong name) is rejected."""
        from hermes_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["solpi_engine_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for hermes_cli.config + solpi_engine_constants so they pick up HERMES_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("hermes_cli.config") or mod == "solpi_engine_constants":
