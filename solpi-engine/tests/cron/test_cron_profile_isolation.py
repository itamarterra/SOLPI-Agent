...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    monkeypatch.setattr(
        solpi_engine_constants, "_get_platform_default_hermes_home", lambda: root
    )
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    assert solpi_engine_constants.get_hermes_home().resolve() == profile_home.resolve()
    assert solpi_engine_constants.get_default_hermes_root().resolve() == root.resolve()
