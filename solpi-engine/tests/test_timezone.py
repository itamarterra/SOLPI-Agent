"""
Tests for timezone support (solpi_engine_time module + integration points).
...
import solpi_engine_time


def _reset_solpi_engine_time_cache():
    """Reset the solpi_engine_time module cache (replacement for removed reset_cache)."""
    solpi_engine_time._cached_tz = None
    solpi_engine_time._cached_tz_name = None
    solpi_engine_time._cache_resolved = False


# =========================================================================
# solpi_engine_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_solpi_engine_time_cache()

    def teardown_method(self):
        _reset_solpi_engine_time_cache()
        os.environ.pop("solpi_engine_timeZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["solpi_engine_timeZONE"] = "Asia/Kolkata"
        result = solpi_engine_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["solpi_engine_timeZONE"] = "UTC"
        result = solpi_engine_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["solpi_engine_timeZONE"] = "America/New_York"
        result = solpi_engine_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["solpi_engine_timeZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="solpi_engine_time"):
            result = solpi_engine_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["solpi_engine_timeZONE"] = "UTC"
        _reset_solpi_engine_time_cache()
        r1 = solpi_engine_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["solpi_engine_timeZONE"] = "Asia/Kolkata"
        _reset_solpi_engine_time_cache()
        r2 = solpi_engine_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["solpi_engine_timeZONE"] = "Europe/London"
        tz = solpi_engine_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["solpi_engine_timeZONE"] = "Not/A/Timezone"
        tz = solpi_engine_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When solpi_engine_timeZONE is set, child process sees TZ env var.
        ...
        #   2) solpi_engine_timeZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("solpi_engine_timeZONE=" + os.environ.get("solpi_engine_timeZONE", "NOT_SET"))'
        )
        ...
        assert "solpi_engine_timeZONE=NOT_SET" in result["output"], (
            "solpi_engine_timeZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When solpi_engine_timeZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("solpi_engine_timeZONE", None)
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        os.environ["solpi_engine_timeZONE"] = "Pacific/Midway"  # UTC-11
        _reset_solpi_engine_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["solpi_engine_timeZONE"] = "US/Eastern"
        _reset_solpi_engine_time_cache()
