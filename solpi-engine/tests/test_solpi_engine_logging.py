"""Tests for solpi_engine_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import solpi_engine_logging
# Use whatever RotatingFileHandler class solpi_engine_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. solpi_engine_logging
...
from solpi_engine_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    solpi_engine_logging._logging_initialized = False
    ...
    solpi_engine_logging._reset_queued_handlers()
    ...
    solpi_engine_logging._install_session_record_factory()
    yield
    ...
    solpi_engine_logging._logging_initialized = False
    solpi_engine_logging.clear_session_context()
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, hermes_home):
        log_dir = solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        assert log_dir == hermes_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in solpi_engine_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, hermes_home):
        ...
        solpi_engine_logging.setup_logging(hermes_home=hermes_home, force=True)
    ...
    def test_custom_log_level(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, hermes_home):
        solpi_engine_logging.setup_logging(
    ...
    def test_writes_to_agent_log(self, hermes_home):
        ...
        test_logger = logging.getLogger("test_solpi_engine_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        solpi_engine_logging.flush_log_queue()
    ...
    def test_warnings_appear_in_both_logs(self, hermes_home):
        ...
        test_logger = logging.getLogger("test_solpi_engine_logging.warning_test")
    ...
    def test_info_not_in_errors_log(self, hermes_home):
        ...
        test_logger = logging.getLogger("test_solpi_engine_logging.info_test")
    ...
    def test_explicit_params_override_config(self, hermes_home):
        ...
        solpi_engine_logging.setup_logging(hermes_home=hermes_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home, mode="cli")
        root = logging.getLogger()
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home, mode="gui")
        root = logging.getLogger()
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, hermes_home):
        ...
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        solpi_engine_logging.set_session_context("abc123")
    ...
    def test_clear_session_context(self, hermes_home):
        ...
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        solpi_engine_logging.set_session_context("xyz789")
        solpi_engine_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, hermes_home):
        ...
        def thread_a():
            solpi_engine_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            solpi_engine_logging.flush_log_queue()

        def thread_b():
            solpi_engine_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        solpi_engine_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        solpi_engine_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = solpi_engine_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = solpi_engine_logging._ComponentFilter(
            solpi_engine_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = solpi_engine_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in solpi_engine_logging.COMPONENT_PREFIXES
        ...
        # break this test.
        gateway_prefixes = solpi_engine_logging.COMPONENT_PREFIXES["gateway"]
    ...
    def test_agent_prefix(self):
        prefixes = solpi_engine_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == solpi_engine_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = solpi_engine_logging.COMPONENT_PREFIXES["cli"]
        assert "hermes_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == solpi_engine_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = solpi_engine_logging.COMPONENT_PREFIXES["gui"]
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        solpi_engine_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, hermes_home):
        solpi_engine_logging.setup_logging(hermes_home=hermes_home)
        solpi_engine_logging.setup_verbose_logging()
        solpi_engine_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        solpi_engine_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = solpi_engine_logging._ComponentFilter(("test",))
        ...
        handlers = [h for h in solpi_engine_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        solpi_engine_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("hermes_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in solpi_engine_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                solpi_engine_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = solpi_engine_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(solpi_engine_logging.sys, "platform", "win32"):
            assert solpi_engine_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not solpi_engine_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(solpi_engine_logging.sys, "platform", "linux"):
            assert not solpi_engine_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, hermes_home):
        level, max_size, backup = solpi_engine_logging._read_logging_config()
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> solpi_engine_logging._ManagedRotatingFileHandler:
        handler = solpi_engine_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that solpi_engine_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        solpi_engine_logging.flush_log_queue()
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = solpi_engine_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, hermes_home):
        ...
        assert any(
            ...
            for h in solpi_engine_logging.rotating_file_handlers()
        )
