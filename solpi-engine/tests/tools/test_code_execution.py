...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    ...
    def test_timezone_shell_quoted_in_remote_execution(self):
        """solpi_engine_timeZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"solpi_engine_timeZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import solpi_engine_constants; print(solpi_engine_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("solpi_engine_constants.py", result["output"])
